...
class TableContentBuilder {
  ...
  buildHeaderRow() {
    ...
    let titleHeaderCell = TableElementGenerator.generateProblemNameElement(
      "Title",
      "javascript:void(0)"
    );
    let acceptanceHeaderCell =
      TableElementGenerator.generateProblemAcceptanceElement("Acceptance");
    let difficultyHeaderCell =
      TableElementGenerator.generateProblemDifficultyElement("Difficulty");
    let frequencyHeaderCell =
      TableElementGenerator.generateProblemAcceptanceElement("Frequency");
    ...
    idHeaderCell.addEventListener(
      "click",
      this.getOnHeaderClickedFunction(IDSorter).bind(this)
    );
    titleHeaderCell.addEventListener(
      "click",
      this.getOnHeaderClickedFunction(NameSorter).bind(this)
    );
    acceptanceHeaderCell.addEventListener(
      "click",
      this.getOnHeaderClickedFunction(AcceptanceSorter).bind(this)
    );
    difficultyHeaderCell.addEventListener(
      "click",
      this.getOnHeaderClickedFunction(DifficultySorter).bind(this)
    );
    frequencyHeaderCell.addEventListener(
      "click",
      this.getOnHeaderClickedFunction(FrequencySorter).bind(this)
    );
  ...
  buildTable(Sortby = NameSorter) {
    ...
    this.isReverseSorted = false;
    let table = TableElementGenerator.generateTableContentElement(
      this.shownData
    );
  ...
  onDurationButtonClicked = (event) => {
    ...
    this.shownData =
      this.durationData[event.currentTarget.getAttribute("duration")];
  ...
  swapContentTableElement = (swapTo) => {
    if (document.getElementById(this.tableId) != undefined)
