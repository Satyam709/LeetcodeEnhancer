...
class LocalStorageFrequencyDataFetcher {
  ...
  fetchData() {
    return this.bufferManager
      .getBufferedData("TableFrequencyData")
  ...
  onDataFetched(data) {
    if (Object.keys(data).length == 0 || data == undefined) {
      return this.bufferManager
        .refreshTableData()
        .then((data) =>
          this.bufferManager.getBufferedData("TableFrequencyData")
        )
...
class LocalStorageEditorialDataFetcher {
  fetchData(problemId) {
    return new GoogleSheetBufferManager()
      .getRowOffsetData()
      .then((data) => this.onPremiumProblemDataFetched(problemId, data));
  }
...
export { LocalStorageFrequencyDataFetcher, LocalStorageEditorialDataFetcher };
